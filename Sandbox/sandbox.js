...
lineItemObjects.forEach(function (obj) {
	...
	switch (obj.u_uc_code) {
		...
		case 'UC-003':
			var allUCRows = [];
			var creditZeroRows = [];   // เก็บเฉพาะรายการ credit = 0
			...
			// -------------------------------------------------------------
			// 🔥 เลือกตัวที่ Credit=0 เพียง 1 ตัว (Order สูงสุด)
			// -------------------------------------------------------------
			var selectedActiveId = null;

			if (creditZeroRows.length > 0) {
				creditZeroRows.sort(function (a, b) {
					return b.order - a.order;   // เรียงจาก order มาก → น้อย
				});

				selectedActiveId = creditZeroRows[0].sys_id; // ตัวที่ order สูงสุด
			}

			// -------------------------------------------------------------
			// 🔥 กำหนด active = TRUE เฉพาะตัวที่เลือก
			// -------------------------------------------------------------
			...
			// -------------------------------------------------------------
			// 🔥 Re-Order ใหม่: inactive ก่อน active
			// -------------------------------------------------------------

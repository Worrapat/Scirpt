...
// #region MockData UC003-1
var usage = [
	{
		u_description_sin: 'Measuring cup',
		u_unit_price: '80',
		u_user_edit_price: '',
		active: false,
		order: 100,
...
<thead>
<tr style="background-color:#297bd8; color:#fff;">
...
<th style="border:1px solid #ddd; padding:8px; text-align:center; width:15%;">Active</th>
</tr>
</thead>
<tbody>
<tr>
<td style="border:1px solid #ddd; padding:8px;">Measuring cup</td>
<td style="border:1px solid #ddd; padding:8px; text-align:center;">80</td>
<td style="border:1px solid #ddd; padding:8px; text-align:center;">-</td>
<td style="border:1px solid #ddd; padding:8px; text-align:center;">FALSE</td>
</tr>
<tr>
<td style="border:1px solid #ddd; padding:8px;">Measuring cup</td>
...
<td style="border:1px solid #ddd; padding:8px; text-align:center;">TRUE</td>
</tr>
</tbody>
</table>`;

var count = 2;
// #endregion MockData UC003-1
...
function buildTicketSummary(rows) {
	...
	rows.forEach(function (r) {
		...
		html +=
			'<tr style="' + rowStyle + '">' +
			'<td style="border:1px solid #ddd; padding:8px">' + descriptionHtml + '</td>' +
			'<td style="border:1px solid #ddd; padding:6px">' + formattedCost + '</td>' +
			'<td style="border:1px solid #ddd; padding:6px">' +
			(r.itemUpdate === true ? '-' : r.unitPrice) +
			'</td>' +
			'<td style="border:1px solid #ddd; padding:6px">' + r.active + '</td>' +
			'<td style="border:1px solid #ddd; padding:6px">' + r.order + '</td>' +
			'</tr>';
	});
	...
	switch (obj.u_uc_code) {
		...
		case 'UC-003':
			...
			while (grUISI.next()) {
				...
				var isActive = (credit === 0);   // Active when credit = 0

				var rowObj = {
					sys_id: grUISI.getUniqueValue(),
					description: grUISI.name.toString(),
					stockInCost: isActive ? obj.u_unit_cost : grUISI.u_stock_in_unit_cost.getDisplayValue(),
					unitPrice: obj.u_unit_cost,
					ucCode: obj.u_uc_code,
			...
			var newCredit = parseInt(obj.u_credit_avaliable || 0);
			var newIsActive = (newCredit === 0);
			...
			var newRow = {
				...
				unitPrice: obj.u_unit_cost,
				ucCode: obj.u_uc_code,
				u_credit_avaliable: newCredit,
				active: newIsActive ? 'true' : 'false',
				itemUpdate: true,
				enableLink: false,
				hasOldRecord: false
			};
			...
			if (newIsActive)
				...
				inactiveRows.push(newRow);

			// ----- Rebuild ordering -----
			// Active first, then Inactive
			var merged = activeRows.concat(inactiveRows);

			var orderCounter = merged.length;
			merged.forEach(function (r) {
				r.order = orderCounter * 100;
				orderCounter--;
			});
			...
			if (producer.u_so_po_no_ != '') {
				...
				while (grUILUPI.next()) {
					...
					currentRows.push({
						...
						itemUpdate: isUpdate, enableLink: true,
						hasOldRecord: true,
					});
				}
			} else {
